...
var index = (options = {}) => {
  ...
  const minifyCSS = (css) => {
    const functions = [];
    return css.replace(/\b(calc|min|max|clamp)\(([^()]*(\([^()]*\))*[^()]*)\)/g, (match) => {
      functions.push(match);
      return `___F${functions.length - 1}___`;
    }).replace(/("([^"\\]|\\.)*"|'([^'\\]|\\.)*')|\/\*[^]*?\*\//g, (_, quoted) => quoted || "").replace(/\s*;\s*(})/g, ";$1").replace(/\s*([*$~^|]?=|[{};,>~]|!important)\s*/g, "$1").replace(/\s*([+-])\s*(?=[^}]*\{)/g, "$1").replace(/([[(])\s+/g, "$1").replace(/\s+([\])])/g, "$1").replace(/({[^}]*?)\s*:\s*/g, "$1:").replace(/^\s+|\s+$/g, "").replace(/(\s)\s+/g, "$1").replace(/\n+/g, " ").replace(/___F(\d+)___/g, (_, index) => functions[index]);
  };
  return {
    name: "import-css",
